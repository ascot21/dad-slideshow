	...
	$.fn.audioSlideshow = function( options ) {
		...
		return this.each(function () {
			...
			var $that = $(this),
				...
				$startButton = $that.find(settings.startSelector),
				$playButton = $that.find(settings.playSelector),
				$pauseButton = $that.find(settings.pauseSelector),
				$controls = $that.find(settings.controlsSelector),
			...
			$startButton.click(function(event){
				...
				$controls.fadeIn()
				$('.audio-slides').fadeIn();
				$jPlayerObj.jPlayer('option', 'fullScreen', true)
				$jPlayerObj.jPlayer("play");
			});
			...
			function setAudioSlide(n){
				if(n != currentSlide){
					if($slides.get(currentSlide)){
						$($slides.get(currentSlide)).fadeOut('fast');
					}

					$($slides.get(n)).fadeIn('fast');
					currentSlide = n;
				}
			}
